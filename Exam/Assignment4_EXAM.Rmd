...
```{r}
Con1$Question.x <- as.factor(Con1$Question.x) 
Con1$SI <- as.factor(Con1$SI)
Con1$CorrectAnswer <- as.factor(Con1$CorrectAnswer)
Con1$Diff <- as.factor(Con1$Diff)
Con1$AgeDif1 <- as.integer(Con1$AgeDif1)
...
prior_2 <- c(
  ...
  prior(student_t(3, 0, 2.5), class = sd, dpar = mu1),
  prior(normal(0, .3), class = b, dpar = mu2),
  prior(student_t(3, 1, 2.5), class = Intercept, dpar = mu2),
  prior(student_t(3, 0, 2.5), class = sd, dpar = mu2)
)
